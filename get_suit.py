...
def get_suit(suit_id, base_dir='./'):
    ...
    try:
        rq_get = get(
            "https://api.bilibili.com/x/garb/mall/item/suit/v2?item_id="
            + str(suit_id)
        )
    except:
   ...
   #"""
    ...
    card_list = [
        ...
        ('bg_preview', res['data']['suit_items']['card_bg'][0]['properties']['image_preview_small'])
    ]
    if 'image_preview_small' in res['data']['suit_items']['card'][0]['properties']:
        card_list.append(
            ('cd_preview', res['data']['suit_items']['card'][0]['properties']['image_preview_small'])
        )
        card_list.append(
            ('image', res['data']['suit_items']['card'][1]['properties']['image'])
        )
    else:
        card_list.append(
            ('cd_preview', res['data']['suit_items']['card'][1]['properties']['image_preview_small'])
        )
        card_list.append(
            ('image', res['data']['suit_items']['card'][0]['properties']['image'])
        )

    #"""
    if not exists(base_dir + '/Card/'):
        makedirs(base_dir + '/Card/')
    for item in card_list:
        try:
            with open(base_dir + '/Card/' + item[0] + '.png',
                      'wb') as card_file:
